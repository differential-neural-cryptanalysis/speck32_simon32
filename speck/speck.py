...
def enc_half_round_nokey(p):
    c0, c1 = p[0], p[1];
    c0 = ror(c0, ALPHA());
    c0 = (c0 + c1) & MASK_VAL;
    return(c0,c1);

def dec_half_round_nokey(c):
    c0, c1 = c[0], c[1];
    c1 = c1 ^ c0;
    c1 = ror(c1, BETA());
    return(c0, c1);

def dec_half_round_withkey(c,k):
    c0, c1 = c[0], c[1];
    c1 = c1 ^ c0;
    c1 = ror(c1, BETA());
    c0 = c0 ^ k;
...
def expand_key_weakkey(k, t, ri, bp, bd):
    ks = [0 for i in range(t)];
    ks[0] = k[len(k)-1];
    l = list(reversed(k[:len(k)-1]));
    for i in range(0, ri):
        l[i%3], ks[i+1] = enc_one_round((l[i%3], ks[i]), i);
    for i in range(ri-1, ri):
        constr = GET1(ks[i+1], bp-1) ^ bd
        ks[i+1] = replace_1bit(ks[i+1], constr, bp)
    for i in range(ri-1, -1, -1):
        l[i%3], ks[i] = dec_one_round((l[i%3], ks[i+1]), i);
    for i in range(t-1):
        l[i%3], ks[i+1] = enc_one_round((l[i%3], ks[i]), i);
    #kbp = GET1(ks[ri], bp-1) ^ GET1(ks[ri], bp)
    #print(kbp)
    #print("====")
    return(ks);
